...
def main():
    ...
    diar_group.add_argument("--diarize", dest="diarize", action="store_true", help="enable diarization")
    diar_group.add_argument("--no-diarize", dest="diarize", action="store_false", help="disable diarization")
    ap.set_defaults(diarize=True)  # you said “always want diarization”
    ap.add_argument("--min-speakers", type=int, default=2, help="min speakers (default: 2)")
    ap.add_argument("--max-speakers", type=int, default=2, help="max speakers (default: 2)")
    ap.add_argument("--timeout", type=float, default=None, help="HTTP timeout seconds (default: 300)")
    args = ap.parse_args()
    ...
    print(f"→ POST {url}")
    print(f"  dial={args.dialect} lang={args.lang} diarize={args.diarize} "
          f"min={args.min_speakers} max={args.max_speakers}")
    try:
        resp = requests.post(url, json=payload, headers={"Content-Type": "application/json"}, timeout=args.timeout)
    ...
    print(f"Speakers list:  {speakers if speakers is not None else []}")
